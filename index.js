      ...
      win.webContents.session.on(
        ...
        (event, item, webContents) => {
          ...
          item.once("done", (event, state) => {
            ...
            // ファイルが正しいかチェックする
            const v =
              item.getTotalBytes() === manual.size &&
...
ipcMain.on("openMSALoginWindow", (ipcEvent, args) => {
  ...
  MSALoginWindow.webContents.on(
    "did-navigate",
    (event, uri, responseCode, statusText) => {
  ...
  MSALoginWindow.loadURL(
    `https://login.microsoftonline.com/consumers/oauth2/v2.0/authorize?prompt=select_account&client_id=${clientID}&response_type=code&scope=XboxLive.signin%20offline_access&redirect_uri=https://login.microsoftonline.com/common/oauth2/nativeclient`
  );
});
