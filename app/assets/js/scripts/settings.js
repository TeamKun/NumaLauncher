      ...
      val.onclick = (e) => {
        ...
        if (Object.keys(ConfigManager.getAuthAccounts()).length === 1) {
          ...
          setOverlayContent(
            ...
            "ケーユーエヌランチャーを使うためには、最低1個のアカウントにログインしている必要があります。<br>ログアウトしたら、もう一度ログインしないといけなくなります。<br><br>本当にログアウトする？",
            "分かってるよそんなこと",
            "やっぱやめとく"
          );
          setOverlayHandler(() => {
...
function loadSelectedServerOnModsTab() {
  ...
  document.getElementById("settingsSelServContent").innerHTML = `
        ...
        <div class="serverListingDetails">
            <span class="serverListingName">${Util.removeOrderNumber(
              serv.getName()
            )}</span>
...
const settingsOptionStandardize = document.getElementById(
  "settingsOptionStandardize"
);
...
function populateSettingsUpdateInformation(data) {
  if (data != null) {
    settingsUpdateTitle.innerHTML = `新しい ${
      isPrerelease(data.version) ? "プレリリース" : "リリース"
    } がご利用可能です`;
    ...
    if (process.platform === "darwin") {
      settingsUpdateButtonStatus(
        'GitHubからダウンロードする<span style="font-size: 10px;color: gray;text-shadow: none !important;">ランチャーを閉じ、ダウンロードしたdmgファイルを起動してください。</span>',
        false,
        () => {
